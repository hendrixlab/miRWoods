...
my $productFileHeader = miRWoods::getProdFileHeader($prodMLParameters);
my $mirBaseGff = miRWoods::$parameters->{mirbaseGff} or die "mirBase gff not loaded.\n";
my $prodTrainFile = miRWoods::$parameters->{productVecTrainFile};
miRWoods::printProdTrainingData($bamList,$genomeDir,$chromLengths,$mirBaseGff,$posClass,$prodTrainFile,$productFileHeader,$prodMLParameters,$parameters);
if ($parameters->{negGff}) {
    my $curatedNegGff = $parameters->{negGff};
    printProdTrainingData($bamList,$genomeDir,$chromLengths,$curatedNegGff,$negClass,$prodTrainFile,$parameters);
} else {
    ...
    miRWoods::addRandomNegFeatures($prodTrainFile,$productVectorFile);
}
