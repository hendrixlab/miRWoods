...
my $hairpinTrainFile = $parameters->{hairpinTrainFile};
my $aucFile = $parameters->{outputPrefix} . "_hairpinAuc.txt";
my $trials = ($parameters->{aucTrials}) ? $parameters->{aucTrials} : 10;
miRWoods::generateRandForestAucFile($hairpinTrainFile,$aucFile,$trials);
