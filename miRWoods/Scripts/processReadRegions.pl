...
if (-e $parameters->{predProductFile}) {
    #if products were found using random forest, use them rather than the ordinary read regions file
    print "Predicted products file found\n";
    print "Using " . $parameters->{predProductRF} . " as the read regions file\n";
    $readRegionsFile = $parameters->{predProductRF};
}
...
if ($parameters->{numProcesses}) {
    miRWoods::processReadRegionsMultiProcess($bamList,$readRegionsFile,$genomeDir,$chromLengths,$parameters);
    miRWoods::combineProcessRRFiles($parameters);
} else {
    miRWoods::processReadRegions($bamList,$readRegionsFile,$genomeDir,$chromLengths,$parameters);
}
